...
def Dense_Model(r,Neurons,batch_size,inputs,lr=1e-4,Memory=.9):
    import keras
    ...
    callbacks = [EarlyStopping(monitor='val_loss', patience=2),
             ModelCheckpoint(filepath='best_model'+str(r)+'.h5', monitor='val_loss', save_best_only=True)]
...
def Train_Steps(epochs,Neurons,X_train,X_test,X_val,y_train,y_test,y_val,i,X_fill,Memory=None):
    ...
    Scorez=[]
    lr = 1e-3
    Mod,callbacks = Dense_Model(i,Neurons,X_train.shape[0],X_train.shape[1],lr=lr,Memory=Memory)
    killscore=0
    killmax = 5
    e = 0
    udate = 3
